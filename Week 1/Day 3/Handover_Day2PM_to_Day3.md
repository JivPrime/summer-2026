# Handover — End of Day 2 (PM) → Start of Day 3
...
**Handover written:** 17 Jun 2026.
**Position:** Just finished **Day 2** = schedule box *"Sun 14 Jun [CATCH-UP] — First Circuit + Variable Resistance + Voltage Dividers"*. Next session is **Day 3** = schedule box *"Mon 15 Jun [CATCH-UP] — Capacitors + RC Time Constants"*.
...
- **Corrected the student's 1W.6 self-test:** their method (meter in parallel with the bottom resistor) was right; the only error was the VOM input resistance — correct is **10,000 Ω/V × 10 V range = 100 kΩ → 6.7 V** (they had mistakenly used 200 kΩ). Their scope part (1 MΩ → 9.5 V) was already right.

**Four bench labs (all built and measured):**
1. **Make:E Expt 3 Part A (KVL):** 9 V + 470 Ω + red LED. Logged V_LED ≈ 2 V, V_s ≈ 8.99 V, V_R ≈ 6.8 V; KVL confirmed (V_LED by KVL = 8.99 − 6.8 = 2.19 V; I ≈ 14.5 mA).
2. **Make:E Expt 3 Part B (Ohm's law via current sweep):** DMM in mA mode **in series**; 1k/1.5k/2.2k/3k → ≈ 9/6/4.1/3 mA; V = I·R returns the supply. (3.3 kΩ substituted by the student's 3 kΩ — value non-critical.)
3. **Divider loading lab:** 10 k / 10 k from 5 V (limit 50 mA). Midpoint 2.50 V → **V_Th = 2.5 V, R_Th = 5 kΩ**. Loaded outputs: 10 k → 1.67 V, 1 k → 0.42 V, 100 Ω → 49 mV. This *is* the Thévenin story (load + R_Th form a divider).
4. **Pot-controlled LED:** 10 k pot ends to 5 V / GND, wiper → 470 Ω → LED → GND. Off below ~2 V at the wiper, full at ~5 V (I ≈ 6.4 mA). Scoped the wiper.

**Deliverable produced:** `EOD_Thevenin_Loading.pdf` + `.tex` (Thévenin recipe box, worked divider→Thévenin example with a circuitikz diagram, condensed lab takeaways). Built with pdflatex (3 passes), both pages rasterised and visually checked.

---

## 2. Big conceptual win — RESOLVED (do not re-litigate in Day 3)

The student fully worked through how a potentiometer behaves. The next session can treat all of this as solid:

- **The wiper (middle pin) is a real node, not a component.** A pot is **one track split by the wiper into two series resistances, R_top + R_bot, always summing to 10 k.** Turning it shifts the split; the wiper voltage is the divider output.
- **The 470 Ω + LED is a *separate branch* hanging off the wiper node** (not inside the pot).
- **The clincher that finally landed:** *parallel = same voltage, always; the voltage "reduction"/droop is a **series** effect across R_top.* When R_top = 0 there is no series resistance to drop voltage, so the wiper sits at the full 5 V; R_bot and the LED branch (in parallel below the wiper) then both see 5 V at different currents. The loading droop seen in the lab happened because **R_top was 10 k**, not 0.
- **Scope grounding:** all scope probe ground clips are internally common and earth-tied → **tie every ground clip to one circuit-ground node**, or two clips will short two points together (this had shorted the LED earlier).
- **LED behaviour:** an LED **clamps voltage** at ≈ 2 V (forward voltage) but does **not** limit current; **current** (heat) is what destroys it; the series resistor's job is to **limit current**. Can't use V/(R + R_LED) because the LED is nonlinear — subtract its ≈ 2 V and use the resistor: I = (V_wiper − 2)/470.
- **Bonus tangent covered:** you *can* make a "pot" from fixed resistors — a **stepped attenuator** (resistor string + a switch/jumper picking the tap). Stepped, not smooth; more resistors → finer steps; this is what hi-fi stepped volume controls and digital pots do.
...
- Also work LAoE **§2W.1–2W.2** (RC filters + the RC step response, worked with numbers) — cements the bench RC charging and gives the step-response maths. *(Frequency-domain filter view of these same circuits comes later: AoE §1.7, Mon 6 Jul.)*
...
## 5. Carried-forward pending items (cross-session)

- **AoE condensation corrections** still outstanding — apply when `AoE_1_2_2_to_1_3_Condensation.tex` source is next available:
...
- **Bench:** Rigol DHO924S scope (1 MΩ input, built-in AWG), Siglent SPD3303X PSU, DSLogic U2Pro16, Extech EX530A + UNI-T UT58C DMMs, Yihua 862BD+, ELEGOO kit, BOJACK 1 % metal-film resistor kit, NUCLEO-F411RE. Devuan Linux (KDE), KiCad 9, etc. Bench LAN: scope 10.0.0.2, PSU 10.0.0.3.
...
- **Condensation work** (when applicable): Book Condensation Contract v2.0, three phases (Content → Diagrams → Delivery), Pass 2 traces **every** T-junction by coordinate, deliver `.pdf` + `.tex`, end with the Kaizen "one thing to improve" question.
...
>
> **PM block:** Make:E Expt 5 (lemon/potato cell). Then RC charging: 10 kΩ + 100 µF, and 1 kΩ + 1 µF, driven by the DHO924S AWG; scope input (CH1) and cap voltage (CH2); measure time to 63 % = τ and compare to RC; use cursors.
>
